...
def parallel_main(rank, world_size, data, num_steps, args, results):
    ...
    try:
        ...
        for i in range(num_steps):
            ...
            grad_and_allreduce_time = (
                end_grad_and_allreduce_time - start_time) * 1000
        ...
        # 收集设备0的计时参数
        if rank == 0:
            results["grad and allreduce"] = sum(
                times["grad and allreduce"]) / len(times["grad and allreduce"])
...
def main():
    ...
    grad_and_allreduce_times = results["grad and allreduce"]
    wait_times = results["wait"]
    step_times = results["step"]

    print(f"平均梯度计算和传播时间: {grad_and_allreduce_times:.2f} ms"
          f", 平均等待时间: {wait_times:.2f} ms"
